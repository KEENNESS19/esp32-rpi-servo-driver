...
#define SERVO_DEBUG_ENABLED true 
#define SERVO_MAX_SYNC_IDS 15
#define SERVO_MAX_SYNC_DATA_LEN 64
#define SERVO_PACKET_HEADER 8   
// 增加缓冲区大小以支持同步写操作 (Header + 15 * (1 + 6) + Checksum) approx 120 bytes
#define SERVO_SYNC_PACKET_MAX_SIZE 256 
...
// 计算校验和
uint8_t servo_calculate_checksum(const uint8_t *data, uint8_t start_idx, uint8_t len)
{
    uint32_t sum = 0;
...
// 发送数据包函数
int servo_send_packet(uart_port_t uart_num, const uint8_t* packet, uint8_t packet_len, uint8_t* response, uint32_t timeout_ms, bool expect_response)
{
...
int servo_parse_response(uint8_t* packet, uint16_t length, 
...
//日志打印
void servo_log_error(uint8_t id, uint8_t error_code)
{
    switch (error_code) {
                    ...
                    uint8_t** responses, uint8_t* response_sizes, uint32_t timeout_ms)
{
    ...
    // 检查responses数组中的每个指针是否已分配内存
    for (int i = 0; i < id_count; i++) {
        if (servo_ids[i] != SERVO_BROADCAST_ID && responses[i] == NULL) {
            ESP_LOGE(TAG, "responses[%d] is NULL for servo ID %d", i, servo_ids[i]);
            return SERVO_STATUS_LENGTH_ERROR;
        }
    }
    ...
    if (expected_response_size == 0) {
        ESP_LOGE(TAG, "Invalid expected response size: 0");
        return SERVO_STATUS_LENGTH_ERROR;
    }
    ...
    // 初始化接收缓冲区
    memset(rx_buffer, 0, expected_response_size);
    ...
    while (offset < rx_bytes && offset + 6 <= rx_bytes) {
        ...
        // 检查数据包完整性
        if (pkt_len < 2) {  // 最小长度：指令+错误码
            ESP_LOGW(TAG, "Invalid packet length %d for ID %d", pkt_len, id);
            offset += 1;
            continue;
        }
        ...
        if (responses[idx] != NULL) {
            ...
            ESP_LOGE(TAG, "responses[%d] is NULL for servo ID %d, skipping data copy", idx, id);
        }
...
int servo_sync_set_position_speed_torque(const uint8_t* servo_ids, uint8_t id_count,
                                        const uint16_t* positions, 
                                        const uint16_t* speeds,
                                        const uint16_t* torques,
                                        uint32_t timeout_ms)
{
    if (servo_ids == NULL || positions == NULL || speeds == NULL || torques == NULL || id_count == 0) {
        ESP_LOGE(TAG, "无效的同步设置参数");
        return SERVO_STATUS_LENGTH_ERROR;
    }
    ...
    // for (int i = 0; i < id_count; i++) {
    //     uint8_t current_mode;
    //     int status = servo_read_data(servo_ids[i], SERVO_REG_MODE, 1, &current_mode, 20);
        
    //     if (status == SERVO_STATUS_OK && current_mode != SERVO_MODE_POSITION) {
    //         servo_set_mode(servo_ids[i], SERVO_MODE_POSITION, 20);
    //     }
        
    //     uint8_t torque_enable[1] = {1};
    //     servo_write_data(servo_ids[i], SERVO_REG_TORQUE_ENABLE, torque_enable, 1, 20);
    // }
    
    // vTaskDelay(pdMS_TO_TICKS(10));
    
    uint8_t sync_data[id_count * 6];
    
    for (int i = 0; i < id_count; i++) {
        uint8_t *servo_data = &sync_data[i * 6];
        uint16_t position = positions[i];
        uint16_t speed = speeds[i];
        uint16_t torque = torques[i];
        
        if (servo_get_END() == 0) {  

            servo_data[0] = position & 0xFF;         
            servo_data[1] = (position >> 8) & 0xFF;  
            
            servo_data[2] = torque & 0xFF;         
            servo_data[3] = (torque >> 8) & 0xFF; 

            servo_data[4] = speed & 0xFF;           
            servo_data[5] = (speed >> 8) & 0xFF;   
            
        } else { 

            servo_data[0] = (position >> 8) & 0xFF;  
            servo_data[1] = position & 0xFF;        
            
            servo_data[2] = (torque >> 8) & 0xFF;   
            servo_data[3] = torque & 0xFF;    

            servo_data[4] = (speed >> 8) & 0xFF;    
    ...
    int status = servo_sync_write(servo_ids, id_count, 
                                 SERVO_REG_GOAL_POSITION_LOW,
                                 sync_data, 6, timeout_ms);
    
    #if SERVO_DEBUG_ENABLED
    if (status == SERVO_STATUS_OK) {
        ESP_LOGI(TAG, "同步设置 %d 个舵机完成", id_count);
        for (int i = 0; i < id_count; i++) {
            ESP_LOGI(TAG, "  舵机 %d: 位置=%u, 速度=%u, 扭矩=%u", 
                    servo_ids[i], positions[i], speeds[i], torques[i]);
        }
    } else {
        ESP_LOGE(TAG, "同步设置失败: %d", status);
    }
    #endif
                                            ...
                                            bool wait_for_user)
{
    ...
    if (wait_for_user) {
        ...
        const uint32_t wait_time_ms = 180000; // 等待180秒
...
int servo_sync_read_positions(const uint8_t* servo_ids, uint8_t id_count,
                             uint16_t* positions, uint32_t timeout_ms)
{
    if (servo_ids == NULL || positions == NULL || id_count == 0) {
        ESP_LOGE(TAG, "无效的同步读取参数");
        return 0;
    }
    ...
    for (int i = 0; i < id_count; i++) {
        responses[i] = (uint8_t*)malloc(2 * sizeof(uint8_t));
        response_sizes[i] = 0;
    }
    
    int success_count = servo_sync_read(servo_ids, id_count,
                                      SERVO_REG_CURRENT_POSITION_LOW, 2,
                                      responses, response_sizes, timeout_ms);
    
    for (int i = 0; i < id_count; i++) {
        if (response_sizes[i] == 2) {
            positions[i] = servo_Transform_8_To_16(responses[i][0], responses[i][1]);
            #if SERVO_DEBUG_ENABLED
            ESP_LOGI(TAG, "舵机 %d 位置: %u", servo_ids[i], positions[i]);
            #endif
        } else if (response_sizes[i] > 0) {
            ESP_LOGW(TAG, "舵机 %d 响应长度错误: %d", servo_ids[i], response_sizes[i]);
            positions[i] = 0xFFFF; // 标记为错误值
        } else {
            ESP_LOGW(TAG, "舵机 %d 无响应", servo_ids[i]);
            positions[i] = 0xFFFF; // 标记为错误值
        }
    }
    ...
    ESP_LOGI(TAG, "同步读取位置完成: %d/%d 成功", success_count, id_count);
    return success_count;
}

int servo_read_speed_with_direction(uint8_t id, uint16_t* speed, bool* direction, 
                                   uint32_t timeout_ms)
{
    ...
    uint8_t speed_data[2];
    int status = servo_read_data(id, SERVO_REG_CURRENT_SPEED_LOW, 2, speed_data, timeout_ms);
    ...
    uint16_t raw_speed = servo_Transform_8_To_16(speed_data[0], speed_data[1]);
    
    *direction = (raw_speed & 0x8000) == 0;  // BIT15=0表示正向，1表示反向
    
    *speed = raw_speed & 0x7FFF;  // 屏蔽方向位
    
    #if SERVO_DEBUG_ENABLED
    ESP_LOGI(TAG, "舵机 %d 速度: %u, 方向: %s (原始: 0x%04X)", 
            id, *speed, *direction ? "正向" : "反向", raw_speed);
    #endif
    
    return status;
}

int servo_sync_read_speeds_with_direction(const uint8_t* servo_ids, uint8_t id_count,
                                         uint16_t* speeds, bool* directions, 
                                         uint32_t timeout_ms)
{
    if (servo_ids == NULL || speeds == NULL || directions == NULL || id_count == 0) {
        ESP_LOGE(TAG, "无效的同步读取参数");
        return 0;
    }
    ...
    for (int i = 0; i < id_count; i++) {
        if (response_sizes[i] == 2) {
            uint16_t raw_speed = servo_Transform_8_To_16(responses[i][0], responses[i][1]);
            
            directions[i] = (raw_speed & 0x8000) == 0;  // BIT15=0表示正向，1表示反向
            speeds[i] = raw_speed & 0x7FFF;  //纯速度数据
            
            #if SERVO_DEBUG_ENABLED
            ESP_LOGI(TAG, "舵机 %d 速度: %u, 方向: %s (原始: 0x%04X)", 
                    servo_ids[i], speeds[i], 
                    directions[i] ? "正向" : "反向",
                    raw_speed);
            #endif
        } else if (response_sizes[i] > 0) {
    ...
    ESP_LOGI(TAG, "同步读取速度完成: %d/%d 成功", success_count, id_count);
    return success_count;
}

int servo_read_temperature(uint8_t id, uint8_t* temperature, uint32_t timeout_ms)
{
    if (temperature == NULL) {
        ESP_LOGE(TAG, "输出参数为空");
        return SERVO_STATUS_LENGTH_ERROR;
    ...
    uint8_t temp_data;
    int status = servo_read_data(id, SERVO_REG_CURRENT_TEMP, 1, &temp_data, timeout_ms);
    ...
    *temperature = temp_data;
    ...
    #if SERVO_DEBUG_ENABLED
    ESP_LOGI(TAG, "舵机 %d 温度: %d°C", id, *temperature);
    #endif
    
    return status;
}

int servo_sync_read_temperatures(const uint8_t* servo_ids, uint8_t id_count,
                                uint8_t* temperatures, uint32_t timeout_ms)
{
    if (servo_ids == NULL || temperatures == NULL || id_count == 0) {
        ESP_LOGE(TAG, "无效的同步读取参数");
        return 0;
    }
    ...
    for (int i = 0; i < id_count; i++) {
        responses[i] = (uint8_t*)malloc(1 * sizeof(uint8_t));
        response_sizes[i] = 0;
    }
    ...
    for (int i = 0; i < id_count; i++) {
        if (response_sizes[i] == 1) {
            temperatures[i] = responses[i][0];
            
            #if SERVO_DEBUG_ENABLED
            ESP_LOGI(TAG, "舵机 %d 温度: %d°C", servo_ids[i], temperatures[i]);
            #endif
        } else if (response_sizes[i] > 0) {
            ESP_LOGW(TAG, "舵机 %d 响应长度错误: %d", servo_ids[i], response_sizes[i]);
            temperatures[i] = 0xFF;  // 标记为错误值
        } else {
            ESP_LOGW(TAG, "舵机 %d 无响应", servo_ids[i]);
            temperatures[i] = 0xFF;  // 标记为错误值
        }
    ...
    ESP_LOGI(TAG, "同步读取温度完成: %d/%d 成功", success_count, id_count);
    return success_count;
}
