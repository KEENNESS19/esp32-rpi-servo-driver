...
icm20948_get_raw_acce(icm20948_handle_t sensor, icm20948_raw_acce_value_t *const raw_acce_value)
{
	...
	ret = icm20948_read(sensor, ICM20948_ACCEL_XOUT_H, data_rd, sizeof(data_rd));
...
esp_err_t icm20948_configure(icm20948_handle_t *handle, icm20948_acce_fs_t acce_fs, icm20948_gyro_fs_t gyro_fs)
{
	esp_err_t ret;
	 ...
	 */
	*handle = icm20948_create(I2C_MASTER_NUM, ICM20948_I2C_ADDRESS_1);
	if (*handle == NULL) {
		ESP_LOGE(TAG, "ICM20948 create returned NULL!");
		return ESP_FAIL;
	}
	icm20948 = *handle; // Keep local static copy for backward compatibility if needed
	ESP_LOGI(TAG, "ICM20948 creation successfull!");

	ret = icm20948_reset(*handle);
	if (ret != ESP_OK)
		return ret;

	vTaskDelay(10 / portTICK_PERIOD_MS);

	ret = icm20948_wake_up(*handle);
	if (ret != ESP_OK)
		return ret;

	// Enable Accelerometer and Gyroscope
	uint8_t pwr_mgmt_2 = 0x00; // Enable all axes
	ret = icm20948_write(*handle, ICM20948_PWR_MGMT_2, &pwr_mgmt_2, 1);
	if (ret != ESP_OK)
		return ret;

	ret = icm20948_set_bank(*handle, 0);
	if (ret != ESP_OK)
		return ret;

	uint8_t device_id;
	ret = icm20948_get_deviceid(*handle, &device_id);
	if (ret != ESP_OK)
		return ret;
	...
	ret = icm20948_set_gyro_fs(*handle, gyro_fs);
	if (ret != ESP_OK)
		return ESP_FAIL;

	ret = icm20948_set_acce_fs(*handle, acce_fs);
	if (ret != ESP_OK)
		return ESP_FAIL;
